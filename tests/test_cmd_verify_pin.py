...
@pytest.mark.description("'VERIFY PIN' is supported and should return 0x9000")
@pytest.mark.test_spec("CHA_APP_VP_OK_01")
@pytest.mark.commands("verify_pin")
@pytest.mark.order("first")
def test_cmd_verify_pin(client):
    authenticate(client)
    pin_digits = bytes([0x01, 0x02, 0x03, 0x04])
...
@pytest.mark.test_spec("CHA_APP_VP_FAIL_01")
@pytest.mark.commands("verify_pin")
def test_cmd_verify_pin_wrong_p1(sender, client):
    authenticate(client)
    wrong_p1 = 0x22
    dummy_data = bytearray([0x00] * 0x04)
    _, sw1, sw2 = sender.build_and_send_apdu_no_throw(
        cla=CLA,
        ins=InsType.VERIFY_PIN,
        p1=wrong_p1,
        p2=P2.P2_DEFAULT,
        data=dummy_data,
    )
...
@pytest.mark.test_spec("CHA_APP_VP_FAIL_02")
@pytest.mark.commands("verify_pin")
def test_cmd_verify_pin_wrong_p2(sender, client):
    authenticate(client)
    wrong_p2 = 0x22
    dummy_data = bytearray([0x00] * 0x04)
    _, sw1, sw2 = sender.build_and_send_apdu_no_throw(
...
@pytest.mark.test_spec("CHA_APP_VP_FAIL_03")
@pytest.mark.commands("verify_pin")
def test_cmd_verify_pin_missing_data(sender, client):
    authenticate(client)
    # Send VERIFY PIN with P1=0x00 and P2=0x00 with lc=0x00 (no data)
    _, sw1, sw2 = sender.build_and_send_apdu_no_throw(
        cla=CLA,
        ins=InsType.VERIFY_PIN,
        p1=P1.P1_DEFAULT,
...
@pytest.mark.test_spec("CHA_APP_VP_FAIL_04")
@pytest.mark.commands("verify_pin")
def test_cmd_verify_pin_wrong_data(client):
    authenticate(client)
    # Send VERIFY PIN with P1=0x00 and P2=0x00 with lc=pin length + 1 (wrong length)
    ...
    _, sw1, sw2 = client.sender.build_and_send_apdu_no_throw(
        cla=CLA, ins=InsType.VERIFY_PIN, p1=P1.P1_DEFAULT, p2=P2.P2_DEFAULT, data=data
    )
    assert_sw(sw1, sw2, StatusWords.WRONG_PARAMETERS)
    check_applet_state(client)
    # Send VERIFY PIN with P1=0x00 and P2=0x00 with pin length > 8 (wrong length)
    ...
    assert_sw(sw1, sw2, StatusWords.WRONG_PARAMETERS)
    check_applet_state(client)
    # Send VERIFY PIN with P1=0x00 and P2=0x00 with pin length < 4 (wrong length)
...
@pytest.mark.description(
    "When PIN data is erroneous, the command should be rejected with 0x63CX where X is the number of remaining attempts"
)
@pytest.mark.test_spec("CHA_APP_VP_FAIL_05")
@pytest.mark.commands("verify_pin")
@pytest.mark.order(-2)
def test_cmd_verify_pin_wrong_pin(client):
    authenticate(client)
    pin_digits = bytes([0x01, 0x02, 0x03, 0x05])
    with pytest.raises(AssertionError) as e:
        client.verify_pin(pin_digits)
    assert str(e.value) == "Status Word: 0x63C2"
    with pytest.raises(AssertionError) as e:
        client.verify_pin(pin_digits)
    assert str(e.value) == "Status Word: 0x63C1"
    check_applet_state(client)


@pytest.mark.description(
    "When the maximum number of PIN verification attempts has been reached, the command should be rejected with 0x6983"
)
@pytest.mark.test_spec("CHA_APP_VP_FAIL_06")
@pytest.mark.commands("verify_pin")
@pytest.mark.order(-1)
def test_cmd_verify_pin_max_attempts(client):
    authenticate(client)
    # The PIN try counter shall still be 1 after the previous test
    pin_digits = bytes([0x01, 0x02, 0x03, 0x05])
    with pytest.raises(AssertionError) as e:
        client.verify_pin(pin_digits)
    assert str(e.value) == "Status Word: 0x6983"
    infos = client.get_status()
